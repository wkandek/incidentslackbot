...
def update_open_status(r):
  ...
  for line in lines:
    if "open" in line:
      ...
      post_notice(INCIDENTCHANNEL, id, "  - ", title, r)
...
if __name__ == "__main__":
  if count_open_status("") > 0:
     ...
     results = app.client.chat_postMessage(channel=INCIDENTCHANNEL, text=msg)
     update_open_status("notify")
     msg = "All open incidents listed"
     results = app.client.chat_postMessage(channel=INCIDENTCHANNEL, text=msg)
  else:
     msg = "No open incidents currently."
     results = app.client.chat_postMessage(channel=INCIDENTCHANNEL, text=msg)
